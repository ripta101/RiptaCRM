...
- Admin config MFE covers Sites (siteKey + embed snippet, regenerate key), Queues (members + auto-popup + capacity overrides), Routing Rules (per site), Pre-chat Fields (per site), and Agent Statuses (the pickable status list + which ones count as available for chats)
- **Pre-chat intake form**: an admin can define Pre-chat Fields per Site (text/number/date/select/textarea/checkbox, reusing Case Management's `FieldType` vocabulary), asked of a visitor before a *brand-new* conversation starts. A site with none configured is completely unaffected — the widget starts the chat immediately, same as before this existed; resuming an existing conversation always skips the form too. Submitted answers are snapshotted onto the conversation (`ConversationIntakeValue`, label captured at submission time) and shown to the agent in a "Visitor details" section above the transcript
