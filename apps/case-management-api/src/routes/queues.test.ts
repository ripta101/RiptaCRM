...
async function createQueue(name = `Queue ${randomUUID()}`) {
  const res = await request(app).post("/api/queues").send({ name });
...
describe("Queue CRUD", () => {
  it("creates, renames, and deletes a queue", async () => {
    ...
    const renamed = await request(app).patch(`/api/queues/${created.body.id}`).send({ name: `${name} (renamed)` });
    expect(renamed.status).toBe(200);
    expect(renamed.body.name).toBe(`${name} (renamed)`);

    const deleted = await request(app).delete(`/api/queues/${created.body.id}`);
    expect(deleted.status).toBe(204);
    const deletedAgain = await request(app).delete(`/api/queues/${created.body.id}`);
  ...
  it("rejects a duplicate queue name with 409", async () => {
    ...
    const second = await request(app).post("/api/queues").send({ name });
  ...
  it("deleting a queue un-assigns it from any stages instead of blocking", async () => {
    ...
    try {
      const deleted = await request(app).delete(`/api/queues/${queue.body.id}`);
      expect(deleted.status).toBe(204);
...
describe("Queue membership", () => {
  it("adds and removes a member", async () => {
    ...
    const added = await request(app).post(`/api/queues/${queue.body.id}/members`).send({ userId: "user-1" });
    expect(added.status).toBe(201);
    expect(added.body.memberUserIds).toEqual(["user-1"]);

    const removed = await request(app).delete(`/api/queues/${queue.body.id}/members/user-1`);
    expect(removed.status).toBe(204);
    const removedAgain = await request(app).delete(`/api/queues/${queue.body.id}/members/user-1`);
    expect(removedAgain.status).toBe(204); // idempotent

    const final = await request(app).get(`/api/queues/${queue.body.id}`);
    expect(final.body.memberUserIds).toEqual([]);
  });

  it("rejects adding the same member twice with 409", async () => {
    const queue = await createQueue();
    await request(app).post(`/api/queues/${queue.body.id}/members`).send({ userId: "user-1" });
    const second = await request(app).post(`/api/queues/${queue.body.id}/members`).send({ userId: "user-1" });
    expect(second.status).toBe(409);
  });

  it("404s when adding a member to a queue that doesn't exist", async () => {
    const res = await request(app).post("/api/queues/does-not-exist/members").send({ userId: "user-1" });
